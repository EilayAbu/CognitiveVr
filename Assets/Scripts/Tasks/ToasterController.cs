...
namespace CognitiveVR.Tasks
{
    ...
    public class ToasterController : MonoBehaviour
    {
        ...
        [Tooltip("Parent GameObject grouping the toaster's powered-on indicator visuals. Enabled while powered on, disabled while off.")]
        [SerializeField] private GameObject _powerIndicatorParent;
        ...
        private void Start()
        {
            if (_powerIndicatorParent != null)
                _powerIndicatorParent.SetActive(_isPoweredOn);
        ...
        private void Update()
        {
            ...
            if (_state >= ToasterState.Cooking && _state <= ToasterState.Burnt && _toastInside && _isPoweredOn)
            {
        ...
        /// (with a boolean argument) for separate on/off buttons. Turning power off while
        /// cooking just pauses progress (like removing the toast) - the current state and
        /// _cookingElapsed are preserved and cooking resumes right where it left off once
